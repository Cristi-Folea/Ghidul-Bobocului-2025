...
function initializeBook() {
  ...
  function closeBook() {
    const isAtBeginning = current_page === 1;
  ...
  function animatie(stanga_dreapta, var_load_page = true) {
    ...
    if (stanga_dreapta === -1) {
      ...
      if (var_load_page) {
        if (current_page - visible_pages >= 0) {
          load_page(pages[current_page - visible_pages + 1]);
        }
    ...
    if (current_page === 3) {
      cuprinsSvg.style.display = "inline-block";
    } else {
      cuprinsSvg.style.display = "none";
    }
  ...
  async function _go_to_page(pageNumber, load_pages = true) {
    const flipDelay = Math.abs(pageNumber - current_page) < 6 ? 100 : 40;
